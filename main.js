...
movies.forEach(item => {
    contentDiv.innerHTML += `<div class="movieItem">
                                <div class="movieImg"> <img src="${item.image}"/></div>
                                <div class="movieTitle">${item.name}</div>
                                
                                
                                <div class="buttomMovieItem">
                                    <div class="movieLength">${item.length}</div>
                                    <div class="movieRank">${item.rank}</div>
                                </div>
                            </div>`
})

var searchText = document.getElementById("textSearchMovies");
searchText.addEventListener("keyup", function(evt){
	var newArray = movies.filter((item) =>  item.name.indexOf(this.value) > -1)
